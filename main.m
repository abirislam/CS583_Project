...
% optimize edge threshold based on number of corners detected
threshold = 0.2;
...
fprintf("Initial threshold = %d, numCorners = %d\n", threshold, numCorners);
while numCorners > 4 && threshold < 0.95
    threshold = threshold + 0.05;
    edges = edge(imResizeGray, 'canny', threshold);
    allCorners = getAllCorners(edges, widthResize, heightResize);
    numCorners = size(allCorners, 1);
    fprintf("Updated threshold = %d, numCorners = %d\n", threshold, numCorners);
end
...
for iter = 1:maxIterations
    ...
    for i = 1:size(validCorners, 1)
        distances = vecnorm(validCorners(i, :) - centroids, 2, 2);
...
% cluster-wise averaging of corners
...
% sort corners clockwise from topLeft
...
sortedCorners = sortCorners(paperCorners);
